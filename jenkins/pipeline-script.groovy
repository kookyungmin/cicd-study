pipeline {
    ...
    stages {
        ...
        stage("Create new Docker Image") {
            steps {
                sh "docker build --no-cache --platform linux/amd64 -t $DOCKERHUB_REPOSITORY:${TAG_NAME} ."
            }
        }
        ...
        stage("Upload Docker Image") {
            steps {
                sh "docker push $DOCKERHUB_REPOSITORY:${TAG_NAME}"
            }
        }
        
        stage("Prune Docker") {
            steps {
                sh "docker rmi $DOCKERHUB_REPOSITORY:${TAG_NAME}"
                ...
                sshagent (credentials: ['jenkins-controller']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ubuntu@${TARGET_HOST} '
                            pwd
                            docker compose down
                            docker rmi ${DOCKERHUB_REPOSITORY}:${params.TAG_NAME}
                            docker pull ${DOCKERHUB_REPOSITORY}:${params.TAG_NAME}
                            docker compose up -d
                        '
                    """
                }
